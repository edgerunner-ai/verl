...
def apply_monkey_patch(
    ...
    # EdgeRunner: Gemma-4 cross-layer KV-sharing under FSDP2 MixedPrecisionPolicy
    ...
    if getattr(model.config, "model_type", None) == "gemma4":
        try:
            ...
            if TextModel is not None and not getattr(TextModel.forward, "_er_kvshare_patched", False):
                ...
                _forward._er_kvshare_patched = True
                TextModel.forward = _forward
                print("Monkey patch Gemma4 shared_kv_states as non-dict leaf (edgerunner)")
